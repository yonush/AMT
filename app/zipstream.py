...
import time
import zipfile
from zipfile import crc32  # type: ignore
from zipfile import stringCentralDir  # type: ignore
from zipfile import stringEndArchive  # type: ignore
from zipfile import stringEndArchive64  # type: ignore
from zipfile import stringEndArchive64Locator  # type: ignore
from zipfile import stringFileHeader  # type: ignore
from zipfile import structCentralDir  # type: ignore
from zipfile import structEndArchive  # type: ignore
from zipfile import structEndArchive64  # type: ignore
from zipfile import structEndArchive64Locator  # type: ignore
from zipfile import structFileHeader  # type: ignore
from zipfile import zlib  # type: ignore
from zipfile import (
    ZIP64_LIMIT,
    ZIP_DEFLATED,
    ZIP_FILECOUNT_LIMIT,
...
try:
    ...
    ZIP64_VERSION = 45  # type: ignore

try:
    from zipfile import BZIP2_VERSION
except ImportError:
    BZIP2_VERSION = 46  # type: ignore

try:
    from zipfile import ZIP_BZIP2
except ImportError:
    ZIP_BZIP2 = 12  # type: ignore

try:
    from zipfile import LZMA_VERSION
except ImportError:
    LZMA_VERSION = 63  # type: ignore

try:
    from zipfile import ZIP_LZMA
except ImportError:
    ZIP_LZMA = 14  # type: ignore

try:
    from zipfile import ZIP_MAX_COMMENT  # type: ignore
except ImportError:
    ZIP_MAX_COMMENT = (1 << 16) - 1  # type: ignore
...
class ZipInfo(zipfile.ZipInfo):
    def __init__(self, *args, **kwargs):
        ...
        self.flag_bits = (
            0x08  # ZIP flag bits, bit 3 indicates presence of data descriptor
        )
    ...
    def DataDescriptor(self):
        ...
        if self.compress_size > ZIP64_LIMIT or self.file_size > ZIP64_LIMIT:
            ...
            fmt = b"<4sLLL"
        return struct.pack(
            fmt, stringDataDescriptor, self.CRC, self.compress_size, self.file_size
        )


class ZipFile(zipfile.ZipFile):
    def __init__(
        self, fileobj=None, mode="w", compression=ZIP_STORED, allowZip64=False
    ):
        ...
        zipfile.ZipFile.__init__(
            self, fileobj, mode=mode, compression=compression, allowZip64=allowZip64
        )
    ...
    def __write(self, filename=None, iterable=None, arcname=None, compress_type=None):
        ...
        if (filename is None and iterable is None) or (
            filename is not None and iterable is not None
        ):
            raise ValueError(
